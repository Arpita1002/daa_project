...
def load_tasks():
    if os.path.exists("tasks.txt"):
        with open("tasks.txt", "r") as file:
            for line in file:
                task_name, deadline, priority, task_status = line.strip().split('|')
                task = [task_name, deadline, priority, task_status == 'True']
                tasks.append(task)
...
def add_task():
    ...
    if priority not in ["1", "2"]:
        messagebox.showerror("Priority Error", "Priority must be 1 (High) or 2 (Low).")
        return
